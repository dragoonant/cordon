...
2. Play the rival node and confirm 20–36% *feels* like a duel rather than a loss, and that FALL BACK reads as a real choice. Tune `RIVAL_HP_RATIO` in `src/sim/run.ts` if it's off — it's a single number now, and `npx tsx tools/balance/rival_probe.ts <seed>` reports the odds.
3. Build 3b (territory maps) — the strategy layer, and now the biggest open item.
4. Then 3c (recruits), and the rival's taunt line / signature Callout (3a leftovers).
