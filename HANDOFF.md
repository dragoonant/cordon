...
| # | Item | Status | Notes |
|---|---|---|---|
| 1 | New items can't be equipped — hangar dropdowns show items but they look greyed/unselectable | **IN PROGRESS (agent) — NOT FIXED as of this note** | Code in `src/ui/hangar/MechCard.tsx` doesn't disable options. Two suspects: (a) dark-theme `<select>` CSS in `src/ui/hangar/hangar.css` making `<option>` text unreadable; (b) `equip()` in `src/sim/hangar.ts` rejecting ("Exceeds generator output" / "Item not in inventory") with a toast the player never notices. Agent brief: make options legible or replace native select, show rejection reason inline, show PWR cost per option. **Verify by**: start a run → `window.__cordon.getState().run.weapons.push('wpn_railgun')` → `bump()` → hangar → equip Railgun on a Halberd. |
| 2 | Nerve is unexplained | **IN PROGRESS (agent)** | Nerve = the resource that pays for Callouts. Regen: +1 per 20s while deployed, +5 on objective complete, +8 to survivors when a squadmate dies, +3 to winners of a battle. Agent brief: tooltips on every Nerve bar, "NERVE 4/10" + cost on every Callout chip, "-3 NERVE" flash on use, Help overlay + `docs/PLAYING.md` text. |
| 3 | Can't move mechs between squadrons; unseating a unit loses it | **IN PROGRESS (agent)** | Brief: pilot+mech = a Unit; click empty cell → picker of available units (remembers last mech); click seated cell → MOVE TO… / MAKE LEADER / UNSEAT; a BENCH strip lists unseated units. Sim already supports it via `assignSlot`/`setLeader` in `src/sim/hangar.ts`. |
...
| 5 | Hover tooltips for weapons/systems (damage, heal, etc.) | **IN PROGRESS (agent)** | Brief: shared `Tooltip` primitive in `@ui/components` + `ItemTooltip` for weapons/systems/frames/callouts; used in hangar, depot, forecast, squad detail, callout chips. |
...
1. Confirm items 1–3, 5 in the browser (agent work may be partial — see `git status`). Fix whatever isn't.
2. Play one run through the rival node; confirm it's winnable with salvaged gear.
